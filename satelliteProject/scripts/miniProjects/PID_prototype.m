...
phi_int = zeros(3,1); % Initial Vector integrating phi over time
...
for i = 1:length(t)
    ...
    ki = 0.5; % Integral Constant (OPTIONAL)
    
    phi = quatLog(q_e); % LOG MAP OF ERROR QUATERNION
    phi_int = phi_int + phi*dt; % LOOP INTEGRATES ERROR OVER TIME

    tau = -kp * phi - kd * omega_k - ki*phi_int; % PID CONTROL LAW

    tau_dist = [1e-4; 0; 0]; % Constant Disturbance
    tau = tau + tau_dist;
...
for i = 1:4
    ...
    if i == 1
        title('Quaternion Components Over Time: PID Control Added');
    end
    grid on;
...
q_final = q_rk4(:,length(t)); % for bugfixes
